...
class Button():
        ...
        # tekst na przycisku, kolor tekstu, funkcja po kliknięciu, czy jest wieloklikiem
        def __init__(self, x, y, width, height, buttons, buttonText='Button', textColor = (20, 20, 20),onclickFunction=None, multiPress=False):
            self.x = x
            self.y = y
            self.width = width
            self.height = height
...
class Application():
    ...
    def __init__(self, game_state):
        ...
        if path.exists(".\data\save.txt"):
            Button(screen_width/2 - start_button_width/2, screen_height/2 - 120,
            start_button_width, 80, self.start_buttons, 'Continue', (255, 255, 255), self.continuegameFunction)

        Button(screen_width/2 - start_button_width/2, screen_height/2 + 15,
                start_button_width, 80, self.start_buttons, 'New Game', (255, 255, 255), self.newstartFunction)
        Button(screen_width/2 - start_button_width/2, screen_height/2 + 150,
                start_button_width, 80, self.start_buttons, 'Quit', (255, 255, 255), self.quitFunction)
        
        preparation_button_width = 360

        Button(screen_width/2 - preparation_button_width/2, screen_height/2 - 30,
                preparation_button_width, 80, self.preparation_buttons, 'Enter labirynth - E', (255, 255, 255), self.enterLabirynthFunction)
        Button(screen_width/2 - preparation_button_width/2, screen_height/2 + 70,
                preparation_button_width, 80, self.preparation_buttons, 'Shop - S', (255, 255, 255), self.shopFunction)
        Button(screen_width/2 - preparation_button_width/2, screen_height/2 + 170,
                preparation_button_width, 80, self.preparation_buttons, 'Quit - Q', (255, 255, 255), self.quitFunction)


        Button(screen_width/2 - start_button_width/2, screen_height/2,
            start_button_width, 80, self.escape_buttons, 'Continue - C', (255, 255, 255), self.afterlabirynthFunction)
        Button(screen_width/2 - start_button_width/2, screen_height/2 + 100,
                start_button_width, 80, self.escape_buttons, 'Quit - Q', (255, 255, 255), self.quitFunction)
        
        Button(screen_width/2 - start_button_width/2, screen_height/2 + 10,
            start_button_width, 80, self.game_over_buttons, 'Restart - R', (255, 255, 255), self.afterlabirynthFunction)
        Button(screen_width/2 - start_button_width/2, screen_height/2 + 100,
                start_button_width, 80, self.game_over_buttons, 'Quit - Q', (255, 255, 255), self.quitFunction)
        
        Button(screen_width/2 - start_button_width-5, screen_height/2 - 10,
            start_button_width, 80, self.confirmation_buttons, 'Normal', (0, 255, 0), self.choosenormalFunction)
        Button(screen_width/2 + 5, screen_height/2- 10,
                start_button_width, 80, self.confirmation_buttons, 'Hard', (255, 0, 0), self.choosehardFunction)
        Button(screen_width/2 - start_button_width/2, screen_height/2 + 80,
                start_button_width, 80, self.confirmation_buttons, 'Cancel', (255, 255, 255), self.cancelFunction)
        
        shop_button_width = 100

        Button(screen_width/2 - preparation_button_width/2, screen_height/2 + 170,
                preparation_button_width, 80, self.shop_buttons, 'Return - R', (255, 255, 255), self.returnFunction)
        Button(screen_width/2 - 3.5*shop_button_width, screen_height/2 - 120,
                shop_button_width, 60, self.shop_buttons, 'Buy', (255, 255, 255), self.oneproductFunction)
        Button(screen_width/2 - 0.5*shop_button_width, screen_height/2 - 120,
                shop_button_width, 60, self.shop_buttons, 'Buy', (255, 255, 255), self.twoproductFunction)
        Button(screen_width/2 + 2.5*shop_button_width, screen_height/2 - 120,
                shop_button_width, 60, self.shop_buttons, 'Buy', (255, 255, 255), self.threeproductFunction)
        Button(screen_width/2 - 3.5*shop_button_width, screen_height/2 + 80,
                shop_button_width, 60, self.shop_buttons, 'Buy', (255, 255, 255), self.fourproductFunction)
        Button(screen_width/2 - 0.5*shop_button_width, screen_height/2 + 80,
                shop_button_width, 60, self.shop_buttons, 'Buy', (255, 255, 255), self.fiveproductFunction)
        Button(screen_width/2 + 2.5*shop_button_width, screen_height/2 + 80,
                shop_button_width, 60, self.shop_buttons, 'Buy', (255, 255, 255), self.sixproductFunction)
    ...
    def _draw_preparation_room(self):
        ...
        ariadne = info_font.render('Total pieces: ' + str(self.totalmoney), True, (255, 255, 255))
        screen.blit(ariadne, (10, 10))
        deaths = info_font.render('Minotaurs killed: ' + str(self.statistics[2]), True, (255, 255, 255))
        screen.blit(deaths, (800, 10))
        escapes = info_font.render(' Times escaped: ' + str(self.statistics[1]), True, (255, 255, 255))
        screen.blit(escapes, (800, 50))
        kills = info_font.render('      Total deaths: ' + str(self.statistics[0]), True, (255, 255, 255))
        ...
        for i in range(12):
            for j in range(12):
                ...
                if room.visited == False:
                    pygame.draw.rect(screen, '#333333',(screen_width/2 - 249 + (j * 42), screen_height/2 - 249 + (i * 42), 36, 36))
                elif room.state == 0:
                    pygame.draw.rect(screen, '#666666', (screen_width/2 - 249 + (j * 42), screen_height/2 - 249 + (i * 42), 36, 36))
                elif room.state == 1:
                    pygame.draw.rect(screen, '#999900',(screen_width/2 - 249 + (j * 42), screen_height/2 - 249 + (i * 42), 36, 36))
                elif room.state == 2:
                    pygame.draw.rect(screen, '#CC0000',(screen_width/2 - 249 + (j * 42), screen_height/2 - 249 + (i * 42), 36, 36))
                elif room.state == 3:
                    pygame.draw.rect(screen, '#703A3A',(screen_width/2 - 249 + (j * 42), screen_height/2 - 249 + (i * 42), 36, 36))
                elif room.state == 4:
                    pygame.draw.rect(screen, '#00CCCC',(screen_width/2 - 249 + (j * 42), screen_height/2 - 249 + (i * 42), 36, 36))
                
        pygame.draw.circle(screen, '#00DE00', (screen_width/2 - 231 + (player_position[1] * 42), screen_height/2 - 231 + (player_position[0] * 42)), 12)
        font_type = 'Arial'
    ...
    def _draw_escaped_screen(self, flag: bool):
        ...
        if not flag:
            ...
            screen.blit(escaped, (screen_width/2 - escaped.get_width()/2, screen_height/2 - escaped.get_height()/2 - 60))
        else:
            killed_minotaur = font.render("You killed Minotaur!", True, (150, 150, 255))
            screen.blit(killed_minotaur, (screen_width/2 - killed_minotaur.get_width()/2, screen_height/2 - killed_minotaur.get_height()/2 - 110))
            congratulations = font.render("Congratulations on completing my game", True, (150, 150, 255))
            screen.blit(congratulations, (screen_width/2 - congratulations.get_width()/2, screen_height/2 - congratulations.get_height()/2 - 45))
        pygame.display.flip()
    ...
    def _confirmationFunction(self, difficulty: int):
        ...
        self.totalmoney = 0
        self.upgrades = {"trap_chance": 0, "lootrooms": 0, "traps": 0, "die_chance": 0, "thread_chance": 0, "torch_time": 0} 
    ...
    def _buyproductFunction(self, number: int): #kup przedmioty, wylicz cene, odejmij kasę
        ...
        while self.running:
            ...
            if self.game_state == "escaped": #ucieczka przez niebieski portal
                ...
                if self.event_active:
                    ...
                    pygame.draw.circle(screen, (100,100,215), (x_coordinate + side_length//2, y_coordinate + side_length//2), 50)
